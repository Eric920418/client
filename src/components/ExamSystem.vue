  ...
  <div class="container mt-2">
    ...
    <div class="card shadow-lg mb-5">
      <div v-if="!oldExamTicket.type" class="card-body">
        <h1 class="card-title text-info">考卷表單</h1>
        <input
          type="text"
          class="form-control mx-3 fs-5 w-25 my-3"
          v-model="ExamTicket.name"
          placeholder="考卷名稱"
        />
        ...
        <div class="mx-3 mt-4 w-50 fs-5">
          <label for="customRange2" class="form-label"
            >難度:
            <div
              v-if="ExamTicket.difficulty == 0"
              class="d-inline ms-1 text-success"
            >
              簡單
            </div>
            <div
              v-if="ExamTicket.difficulty == 1"
              class="d-inline ms-1 text-primary"
            >
              普通
            </div>
            <div
              v-if="ExamTicket.difficulty == 2"
              class="d-inline ms-1 text-secondary"
            >
              困難
            </div>
            <div
              v-if="ExamTicket.difficulty == 3"
              class="d-inline ms-1 text-warning"
            >
              非常困難
            </div>
            <div
              v-if="ExamTicket.difficulty == 4"
              class="d-inline ms-1 text-danger"
            >
              超級困難
            </div>
          </label>
        ...
        <div class="d-flex">
          <input
            type="date"
            class="form-control mx-3 my-2 w-25 fs-5"
            v-model="ExamTicket.date"
          />
          <input
            type="time"
            class="form-control mx-3 my-2 w-25 fs-5"
            v-model="ExamTicket.time"
          />
        </div>
        <label for="inputState" class="mx-3 mt-2 w-50 form-label fs-5"
          >題數</label
        ...
        <div class="d-flex flex-wrap justify-content-center mt-4">
          <div
            ...
            style="width: 90%"
            v-for="(q, index) in ExamTicket.question"
            :key="index"
          >
            <div class="card-body border-0">
              ...
              <div class="d-flex">
                ...
                <div class="ms-auto">
                  ...
                  <button class="btn btn-danger" @click="clearQuestion(index)">
                    清空
                  </button>
                </div>
              </div>
        ...
        <div class="d-flex justify-content-end">
          <button class="btn btn-success" @click="pushNewTicket">
            發送考卷
          </button>
        </div>
      </div>
      <div v-if="oldExamTicket.type" class="card-body">
        ...
        <div class="mx-3 mt-4 w-50 fs-5">
          <label for="customRange2" class="form-label"
            >難度:
            <div
              v-if="oldExamTicket.difficulty == 0"
              class="d-inline ms-1 text-success"
            >
              簡單
            </div>
            <div
              v-if="oldExamTicket.difficulty == 1"
              class="d-inline ms-1 text-primary"
            >
              普通
            </div>
            <div
              v-if="oldExamTicket.difficulty == 2"
              class="d-inline ms-1 text-secondary"
            >
              困難
            </div>
            <div
              v-if="oldExamTicket.difficulty == 3"
              class="d-inline ms-1 text-warning"
            >
              非常困難
            </div>
            <div
              v-if="oldExamTicket.difficulty == 4"
              class="d-inline ms-1 text-danger"
            >
              超級困難
            </div>
          ...
          <input
            ...
            max="4"
            id="customRange2"
            v-model="oldExamTicket.difficulty"
          />
        </div>
        <label for="inputState" class="mx-3 mt-2 w-50 form-label fs-5"
          >期限</label
        ...
        <div class="d-flex">
          <input
            type="date"
            class="form-control mx-3 my-2 w-25 fs-5"
            v-model="oldExamTicket.date"
          />
          <input
            type="time"
            class="form-control mx-3 my-2 w-25 fs-5"
            v-model="oldExamTicket.time"
          />
        </div>
        <label for="inputState" class="mx-3 mt-2 w-50 form-label fs-5"
          >題數</label
        ...
        <div class="d-flex flex-wrap justify-content-center mt-4">
          <div
            class="card m-2 border-3"
            style="width: 90%"
            v-for="(q, index) in oldExamTicket.question"
            :key="index"
          >
            <div class="card-body border-0">
              <input
        ...
        <div class="d-flex justify-content-end">
          ...
          <button class="btn btn-success" @click="pushTicket">發送考卷</button>
        </div>
      </div>
    </div>
  </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body text-black">
          <table class="table table-hover align-middle">
            ...
            <tbody>
              <tr
                v-for="(exam, index) in AllExamTicket"
                :key="index"
              ...
              >
                ...
                <td>
                  <button class="btn btn-danger" @click="deleteExam(index)">
                    刪除
                  </button>
                </td>
              </tr>
...
export default {
  ...
  methods: {
    addOption(index) {
      const newOption =
        "選項" + (this.ExamTicket.question[index].options.length + 1);
    ...
    saveTicket() {
      ...
      this.$axios
        .post("/exam/save", this.ExamTicket, {
          headers: {
    ...
    saveOldTicket() {
      ...
      var storedToken = localStorage.getItem("token");
      this.$axios
        .put(`/exam/${this.oldExamTicket._id}`, this.oldExamTicket, {
          headers: {
            Authorization: `Bearer ${storedToken}`,
            "Content-Type": "application/json",
          },
    ...
    pushOnlyOne(user) {
      ...
      var storedToken = localStorage.getItem("token");
      this.$axios
        .post(`/exam/${user}`, this.oldExamTicket, {
          headers: {
            Authorization: `Bearer ${storedToken}`,
            "Content-Type": "application/json",
          },
    ...
    deleteExam(index) {
      var storedToken = localStorage.getItem("token");
      this.$axios
        .delete(`/exam/${this.AllExamTicket[index]._id}`, {
          headers: {
            Authorization: `Bearer ${storedToken}`,
            "Content-Type": "application/json",
          },
