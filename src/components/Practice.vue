        ...
        <div class="card-body p-0">
            <div class="mx-auto " style="width: 90%;">
                <div class="d-flex justify-content-between mb-3 mt-4">
                    <input type="text" class="form-control fs-5 me-1 my-3 " v-model="task.taskName"  placeholder="任務名稱">
                    <input type="text" class="form-control fs-5 ms-1  my-3 " v-model="task.class"  placeholder="班級">                    
                </div>
                <div class="d-flex justify-content-between mb-3">
                    <div class="card  p-3 " style="width: 80%; margin: 0 1%;">
                        <div v-for="(guide, index) in task.guide" :key="index" :value="guide"  class="d-flex justify-content-between my-2 w-100">
                            <div class="w-100">
                                <input type="text" class="form-control fs-5" v-model="task.guide[index]"   :placeholder="`子任務${index+1}`">
                            </div>
                            <div class="d-flex align-items-end">
                                
                                <button  type="button" class="btn btn-danger ms-2 py-2" @click="remove('guide',index)"><i class="fa-solid fa-trash"></i></button>
                            </div>
                        </div>
                        <button  type="button" class="btn btn-primary ms-2 py-2" @click="add('guide')"><i class="fa-solid fa-plus"></i></button>
                    </div>
                    <div class="card  p-3 " style="width: 80%; margin: 0 1%;">
                        <div v-for="(target, index) in task.target" :key="index" :value="target" class="d-flex justify-content-between my-2 w-100">
                            <div class="w-100 ">
                                <input type="text" class="form-control fs-5 " v-model="task.target[index]"  :placeholder="`目標${index+1}`">
                            </div>
                            <div class="d-flex align-items-end">
                                <button type="button" class="btn btn-danger ms-2 py-2" @click="remove('target',index)"><i class="fa-solid fa-trash"></i></button>
                            </div>
                        </div>
                    <button  type="button" class="btn btn-primary ms-2 py-2" @click="add('target')"><i class="fa-solid fa-plus"></i></button>
                    </div>
                </div>
                <div class="card p-3 w-100">
                    <div v-for="(question, index) in task.question" :key="index" :value="question" class="d-flex justify-content-between my-2">
                    <div class="w-100">
                        <input type="text" class="form-control fs-5" v-model="task.question[index]"  :placeholder="`實作問題${index+1}`">
                    </div>
                    <div class="d-flex align-items-end">
                        <button  type="button" class="btn btn-danger ms-2 py-2" @click="remove('question',index)"><i class="fa-solid fa-trash"></i></button>
                    </div>
                </div>
                <button  type="button" class="btn btn-primary ms-2 py-2" @click="add('question')"><i class="fa-solid fa-plus"></i></button>
                </div>

                <br>
                <h4 class="ms-1" style="margin-top: 50px;">教材PPT</h4>
                <div class="card p-3 w-100">
                    <input class="form-control" type="file" id="ppt" @change="handleFileUpload"accept="application/pdf" />
                </div>
                <!-- <div id="presentation">
